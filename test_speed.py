...
def main(args):
    ...
    for detection, result_path, EdgeIdx, EdgeAttr in tqdm(zip(args.Detections, args.ResultPath, args.EdgeIdxs, args.EdgeAttributes)):
        ...
        end_time=time.time()
        method_1_times.append(abs(end_time-start_time))
        if(args.UseMethod1):        
            ...
            end_time=time.time()
            method_2_times.append(abs(end_time-start_time))
        ...
        if(args.UseMethod2):        
            ...
            for frame_num in tqdm(range(int(detection_df.fn.min()), int(detection_df.fn.max()+1))): #looping from df.fn.min to df.fn.max
                ...
                trackers = mot_tracker.update(dets)
            end_time=time.time()
            method_3_times.append(abs(end_time-start_time))
    for i, path in enumerate(args.EvaluatePaths):
        with open(path, 'w') as f:
            if(i==0):
                print(sum(method_1_times)/len(method_1_times), file=f)
            if(i==1):
                print(sum(method_2_times)/len(method_2_times), file=f)
            if(i==2):
                print(sum(method_3_times)/len(method_3_times), file=f)
...
def complete_args(args):
    ...
    for d in glob(os.path.join(args.Dataset,"*")):
        if(os.path.isdir(d) and "results" not in d.lower()):
            ...
            res_paths=[]
            result_path_1=os.path.join(args.Dataset,"Results",path_name, "result0_5.txt")
            res_paths.append(result_path_1)
            if(args.UseMethod1):
                result_path_2=os.path.join(args.Dataset,"Results",path_name, "result1_5.txt")
                res_paths.append(result_path_2) 
            if(args.UseMethod2):
                result_path_3=os.path.join(args.Dataset,"Results",path_name, "result2_5.txt")
    ...
    args.EvaluatePaths.append(os.path.join(args.Dataset,"Results","Evaluate","evaluate0_runtime.txt"))
    if(args.UseMethod1):
        args.EvaluatePaths.append(os.path.join(args.Dataset,"Results","Evaluate","evaluate1_runtime.txt"))
    if(args.UseMethod2):
        args.EvaluatePaths.append(os.path.join(args.Dataset,"Results","Evaluate","evaluate2_runtime.txt"))
    return args
